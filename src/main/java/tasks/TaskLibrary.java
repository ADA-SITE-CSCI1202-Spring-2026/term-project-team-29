...
public class TaskLibrary {
	...
	static {
		...
		TASK_FACTORIES.add(() -> new EngineeringTask("Fix Loose Solar Panel Cable", 5, 2, 1, 0, 1));
        // "NOTICE: Power generation slightly reduced. Ignoring may lead to gradual energy shortages."
		TASK_FACTORIES.add(() -> new EngineeringTask("Calibrate Power Generator", 4, 1, 1, 0, 1));
        // "WARNING: Generator efficiency dropping. Long-term neglect increases risk of overload."
		TASK_FACTORIES.add(() -> new EngineeringTask("Replace Broken Circuit Fuse", 3, 1,  1, 0, 1)); // easy
        // "NOTICE: Minor electrical instability detected. Could escalate into system-wide outages."

		TASK_FACTORIES.add(() -> new EngineeringTask("Repair Solar Array Damage", 12, 5, 2, 1, 3));
        // "WARNING: Solar array damaged. Energy production significantly reduced."
		TASK_FACTORIES.add(() -> new EngineeringTask("Restore Power Grid Stability", 15, 6,  2, 1, 4));
        // "ALERT: Power grid unstable. Random system shutdowns may occur."
		TASK_FACTORIES.add(() -> new EngineeringTask("Fix Water Recycling Pump", 10, 4, 2, 1, 3)); // medium
        // "WARNING: Water recycling efficiency reduced. Supplies will drain faster."

		TASK_FACTORIES.add(() -> new EngineeringTask("Rebuild Reactor Cooling System", 25, 10, 4, 2, 6));
        // "CRITICAL: Reactor overheating. Ignoring may result in catastrophic meltdown."
		TASK_FACTORIES.add(() -> new EngineeringTask("Emergency Reactor Shutdown", 30, 12,  5, 2, 7));
        // "EMERGENCY: Reactor instability detected. Immediate shutdown required to prevent explosion."
		TASK_FACTORIES.add(() -> new EngineeringTask("Repair Meteor-Damaged Power Core", 28, 11,  4, 2, 6)); // hard
		...
		TASK_FACTORIES.add(() -> new LifeSupportTask("Fix Minor Oxygen Leak", 5, 1, 2, 1, 0, 2));
		...
		TASK_FACTORIES.add(() -> new ResearchTask("Analyze Soil Sample", 3, 1,  1, 0, 1));
        // "INFO: Soil composition unknown. Ignoring slows future terraforming progress."
		TASK_FACTORIES.add(() -> new ResearchTask("Run Basic Water Test", 2, 1,  1, 0, 1));
        // "NOTICE: Water purity uncertain. Contaminated water may affect crew health."
		TASK_FACTORIES.add(() -> new ResearchTask("Calibrate Lab Sensors", 2, 1,  1, 0, 1)); // easy
        // "INFO: Sensor accuracy degraded. Research results may become unreliable."

		TASK_FACTORIES.add(() -> new ResearchTask("Study Microbial Life", 8, 3,  2, 1, 3));
        // "WARNING: Unknown microorganisms detected. Potential biological threat."
		TASK_FACTORIES.add(() -> new ResearchTask("Develop Plant Growth Formula", 7, 3,  2, 1, 3));
        // "INFO: Food production efficiency limited. Ignoring delays sustainability."
		TASK_FACTORIES.add(() -> new ResearchTask("Analyze Radiation Levels", 9, 4,  2, 1, 4)); // medium
        // "WARNING: Radiation spikes detected. Long-term exposure risk increasing."

		TASK_FACTORIES.add(() -> new ResearchTask("Create Synthetic Oxygen Generator", 15, 6,  3, 2, 5));
        // "CRITICAL PROJECT: Alternative oxygen source required. Failure limits survival options."
		TASK_FACTORIES.add(() -> new ResearchTask("Alien Virus Containment Study", 18, 7,  4, 2, 6));
        // "BIOHAZARD: Unknown virus detected. Unchecked spread could wipe out crew."
		TASK_FACTORIES.add(() -> new ResearchTask("Terraforming Experiment", 20, 8, 4, 2, 7)); // hard
